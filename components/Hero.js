...
const HomeHero = ({ data }) => {
  ...
  // Cycle through the items automatically every 6 seconds
  useEffect(() => {
  ...
  return (
    <div id="hero" className="relative md:h-[80vh] mt-8 overflow-hidden">
      {data.map((item, i) => {
        ...
        return (
          <div
            ...
            className={`transition-opacity duration-1000 ${
              isActive ? "opacity-100 z-10" : "opacity-0 z-0 hidden"
            }`}
          >
            <div className="relative flex flex-col md:flex-row justify-center items-center h-full text-center text-white md:text-left px-6 md:px-12">
              <img
                className="absolute inset-0 w-full h-full md:h-[80vh] object-cover opacity-50 blur-sm"
              ...
              <Link href={`/${item.media_type}/${item.id}`}>
                <img
                  className="rounded-lg drop-shadow-2xl w-40 h-56 md:w-72 md:h-96 lg:w-80 lg:h-[30rem] mt-10 md:mt-0 hover:scale-105 transition-all duration-500 ease-in-out relative z-10"
              ...
              <div className="relative z-10 md:ml-20 my-10 md:my-20 mx-2 md:pl-2 text-center md:text-left">
                <h1 className="max-w-full text-3xl md:text-5xl xl:text-6xl font-extrabold justify-center">
                ...
                <Link href={`/${item.media_type}/${item.id}`}>
                  <button className="mt-8 px-6 py-4 bg-primary rounded-lg hover:bg-sky-500 transition-all duration-300 ease-in-out font-semibold">
                    View Details
                  </button>
                </Link>
              </div>
